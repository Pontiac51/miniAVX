...
void onOBSSelect() {
    drawLED = &drawLEDObsGps;
    onMainSelect = &onObsSelect12;
    onRightSelect = &onAPGPSToggle;
    onLeftSelect = &onAPNAVToggle;
    onLeftCW = &onOBSIncrease;
    onLeftCCW = &onOBSDecrease;
    onRightCW = NULL;
    onRightCCW = NULL;
}
...
void setup() {
  //menu
  addMenuItem("ALT - gSPD", &onAltSpeedSelect);
  ...
  addMenuItem("OBS 1 - GPS", &onOBSSelect);
...
void onSelectRadarAlt(){
  if (!itemsMain[selItem].option){ // 1
    itemsMain[selItem].entry = "rALT - SPD";
    itemsMain[selItem].option = true;
  } else { // 2
    itemsMain[selItem].entry = "ALT - gSPD";
...
void onObsSelect12(){
  if (!itemsMain[selItem].option){ // 1
    itemsMain[selItem].entry = "OBS 2 - GPS";
    itemsMain[selItem].option = true;
  } else { // 2
    itemsMain[selItem].entry = "OBS 1 - GPS";
    itemsMain[selItem].option = false;
  }
  updateOLED();
}
...
void drawLEDObsGps(){
  ...
  if (!itemsMain[selItem].option){ // 1
    obs = connectorRX.getNavObs1();
  } else { // 2
    obs = connectorRX.getNavObs2();
  }
  lc.setChar(0,7,' ',false);
  lc.setChar(0,6,(obs/100)%10,false);
  lc.setChar(0,5,(obs/10)%10,false);
  lc.setChar(0,4,obs%10,connectorRX.getAPNav1LockOn());
...
void drawLEDAltSpd(){
  int alt = 0;
  int spd = 0;
  if (!itemsMain[selItem].option){ // alt ground speed
...
void drawLEDAltVs(){
  ...
  if (vs < 0){
    if (abs(vs) >= 1000){
      lc.setChar(0,3,'-',false);
      lc.setChar(0,2,(abs(vs)/1000)%10,false);
